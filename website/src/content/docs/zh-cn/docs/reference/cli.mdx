...
- Bub 获取 provider 的模型列表，异步超时为 10 秒，随后支持搜索选择或手动输入模型。阻塞式 SDK 调用可能等待更久。检查只验证模型列表接口可访问，不发起模型生成请求。获取失败时可修改 URL/key、重试或手动输入；不支持模型发现的 provider 也可以手动配置。
- 实际连接使用 OpenAI OAuth 时会跳过模型发现，直接手动输入模型。官方 URL 保持隐式默认值时仍可使用 OAuth；仅打开连接编辑步骤而未修改默认 URL 不会改变认证路由。
