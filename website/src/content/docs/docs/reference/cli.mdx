...
- Bub fetches the provider's model list with a 10-second timeout, then offers searchable model selection and manual entry. This checks access to the models endpoint; it does not test a model completion. If discovery fails, edit the URL/key, retry, or enter a model ID manually. Providers without model discovery also support manual entry.
- When the effective connection uses OpenAI OAuth, model discovery is skipped and you enter a model ID manually. Leaving the official URL at its implicit default, including after editing connection details, keeps OAuth routing available.
